program grid2d
  ...
  real(dp), parameter :: h = 1.2_dp*0.05_dp, rho0 = 1.0_dp, gam = 7.0_dp
  real(dp), parameter :: cs = 10.0_dp, p0 = rho0*cs**2/gam, mu0 = 1.0d0/160.d0
  character(len=80) :: kernelname = 'm4',dir = '/home/tmattner/sph/',job = 'couette'
 ...
 ! fully developed flow profile
        x0 = dx(2)*(3) + 0.5_dp*dx(2)
        x1 = dx(2)*(n(2)-4) + 0.5_dp*dx(2)
        u(1,k) = 1.0_dp - (x0 - x(2,k))/(x0 - x1)
! end fully developed flow          
        end if
     end do
  end do 
